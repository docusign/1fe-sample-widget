...
test.describe('Widget Tests', () => {
  ...
  test('should validate basic browser functionality', async ({ page }) => {
    ...
    await page.goto('data:text/html,<html><head><title>Browser Test</title></head><body><div id="test">Playwright is working!</div></body></html>');
  ...
  test.skip('should load the widget', async ({ page }) => {
    ...
    await expect(page.locator('text=My amazing component from app2 is mounted!')).toBeVisible();
  });
});